...
const ticketController = {
  async addTicket(req, res, next) {
    try {
      ...
      return Response.send(res, 201, saveTicket, "Ticket created successfully");
    } catch (error) {
      return next(error);
    }
  },
  ...
  fetchOne: async (req, res, next) => {
    ...
    try {
      ...
      return Response.send(res, 200, data, "Ticket retrived successfully");
    } catch (err) {
      return next(err);
    }
  },
  ...
  deleteOne: async (req, res, next) => {
    try {
      const { org_id } = req.query;
      const { ticket_id } = req.query;
      const data = await Ticket.delete({ id: ticket_id }, org_id);
